...
// 
const ratingStar=(rating=0,parent="star_item",child="path")=>{
    // let ratingFloor=Math.floor(rating)
    console.log(props.rating,'asldfjlasjdf')
    ...
    // //start from back
    // for(let q=star.length-1;q>=0;q--){
    //     star[q].getElementsByTagName(child)[0].setAttribute('stroke',"#FFCE3D")
    // }
}
// const calculateRating=computed(()=>{
//     let ratingF=Math.floor(props.rating)
//     validation.ratingStar(ratingF,`star_item_${name.value}`)
//     return 5
// })
// onMounted(()=>{
//     validation.ratingStar(ratingF.value,`star_item_${name.value}`)
// })
// onUpdated(()=>{
//     validation.ratingStar(ratingF.value,`star_item_${name.value}`)
// })
...
<template>
    <div :id="`star_list_${name}`" class="star">
        <svg v-for="(star,index) in 5" :key="`${index}_${name}`" :class="`star_item_${name}`" xmlns="http://www.w3.org/2000/svg" class="star_item"   width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path 
